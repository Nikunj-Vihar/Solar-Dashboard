...
describe("buildTrendData", () => {
  it("passes through one point per day at day granularity", () => {
    ...
    const result = buildTrendData(readings, "day");
    expect(result).toHaveLength(2);
    expect(result[0]).toMatchObject({ date: "2026-01-01", actualKwh: 88 });
  });

  it("sums 7 days into one week bucket", () => {
  ...
  it("returns an empty array for no readings", () => {
    expect(buildTrendData([], "day")).toEqual([]);
  });
